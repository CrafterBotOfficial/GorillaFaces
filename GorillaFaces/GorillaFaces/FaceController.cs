...
{
    public const string PropertyKey = "GorillaFace";

    private static bool facesLoaded;
    public static Dictionary<string, CustomFace> CachedFaces = new();
    ...
    {
        if (player.CustomProperties.TryGetValue(PropertyKey, out object value)) EquipFace(player, (string)value);
    ...
    public static void UpdateCustomProperties()
    {
        ...
        var hashTable = new ExitGames.Client.Photon.Hashtable() { { PropertyKey, selectedFace } };
    ...
    private static string GetId(Package package)
    {
        ...
        return Id.Length > MaxCharacters ? Id.Substring(0, MaxCharacters) : Id; 
    }
}